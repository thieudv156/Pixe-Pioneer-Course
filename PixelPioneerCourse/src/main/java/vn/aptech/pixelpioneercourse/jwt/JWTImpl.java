...
public class JWTImpl implements JWT{
    ...
    public String encode(int id, List<String> roles, LocalDateTime expiredAt, String secret) {
        ...
        var accessToken = Jwts.builder().subject(String.valueOf(id))
                .claim("roles", String.join(",", roles)).expiration(Date.from(expiredAt.atZone(ZoneId.systemDefault()).toInstant()))
    ...
    public Authorized decode(String token, String secret) {
        ...
        var id = decode.getPayload()
                .getSubject();

        var rolesString = decode.getPayload()
                .get("roles")
                .toString();
